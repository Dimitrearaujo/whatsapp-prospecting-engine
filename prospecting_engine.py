...
def _get_pending_prospects(conn: sqlite3.Connection, limit: int, segment: str = None):
    """Retorna prospects pendentes de envio. Nunca-contatados vêm primeiro, depois por score."""
    base = """
        SELECT * FROM prospects
        WHERE current_step < 7
          AND responded = 0
          AND status NOT IN ('Sem Interesse', 'Convertido', 'sequencia_concluida', 'sem_whatsapp')
          AND (last_sent_at IS NULL OR datetime(last_sent_at) < datetime('now', '-24 hours'))
    """
    order = " ORDER BY (last_sent_at IS NULL) DESC, score DESC, current_step ASC LIMIT ?"
    if segment:
        return conn.execute(
            base + " AND segment = ?" + order,
            (segment, limit)
        ).fetchall()
...
def _mark_no_whatsapp(conn: sqlite3.Connection, prospect_id: int, phone: str) -> None:
    """Marca lead como sem WhatsApp ativo — nunca mais tenta enviar."""
    conn.execute(
        "UPDATE prospects SET status = 'sem_whatsapp', updated_at = ? WHERE id = ?",
        (now_iso(), prospect_id),
    )
    conn.commit()
    LOG.warning("  Numero %s nao tem WhatsApp — marcado como sem_whatsapp.", mask_phone(phone))
...
def _send_whatsapp(phone: str, message: str, config: dict, dry_run: bool) -> tuple:
    """Envia mensagem via Evolution API.
    Retorna (ok: bool, no_whatsapp: bool).
    no_whatsapp=True indica que o número não existe no WhatsApp (400 exists:false).
    """
    if dry_run:
    ...
    # Detecta numero inexistente no WhatsApp (400 + "exists":false na resposta da API)
    no_whatsapp = False
    if status == 400:
        # resp["error"] é a string JSON bruta da API — checa direto, sem re-serializar
        raw_err = resp.get("error", "") if isinstance(resp, dict) else str(resp)
        if '"exists":false' in raw_err or '"exists": false' in raw_err:
            no_whatsapp = True

    LOG.error("  Falha WhatsApp -> %s | status=%s | resp=%s", mask_phone(phone), status, str(resp)[:200])
    return False, no_whatsapp
...
def action_search(config: dict, limit: int) -> None:
    ...
    try:
        new_count = apify_run_search(config=config, limit=limit)
    except Exception as exc:
    ...
    LOG.info("Busca concluida: %d novos leads inseridos.", new_count)


def action_send(config: dict, dry_run: bool, limit: int, segment: str = None) -> None:
    """Executa os disparos da fila de prospects."""
    mode = "[DRY-RUN] " if dry_run else ""
    LOG.info("=== %sENVIO DE MENSAGENS ===", mode)
    if segment:
        LOG.info("Filtro de segmento: %s", segment)

    conn = _get_conn()
    _ensure_db(conn)
    prospects = _get_pending_prospects(conn, limit, segment)
    ...
    for row in prospects:
        ...
        # Double-check atomico: re-lê o step do banco para evitar envio duplo
        # se duas instâncias do engine rodarem ao mesmo tempo (ex: 08:00 e 09:00)
        step_col = _step_col(next_step)
        fresh = conn.execute(
            f"SELECT {step_col}, last_sent_at FROM prospects WHERE id = ?", (pid,)
        ).fetchone()
        if fresh and fresh[0]:
            LOG.warning("  Step %d ja registrado por outro processo. Pulando.", next_step)
            skip_count += 1
            continue
        ...
        # Gera mensagem
        short_name = business_name.split("|")[0].split(" - ")[0].strip()
        lead_data = {
            "business_name": short_name,
            "first_name": short_name.split()[0] if short_name else "voce",
            "segment": segment,
        ...
        if channel == "whatsapp" and phone:
            ...
            ok, no_whatsapp = _send_whatsapp(phone, message, config, dry_run)
        ...
        if no_whatsapp and not dry_run:
            _mark_no_whatsapp(conn, pid, phone)
            skip_count += 1
...
def action_daily(config: dict, dry_run: bool, limit: int, segment: str = None) -> None:
    ...
    action_send(config, dry_run, limit, segment)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Filtra disparos por segmento (ex: 'clinicas veterinarias')",
    )
    ...
    if args.search:
        ...
        action_send(config, args.dry_run, args.limit, args.segment)
        ...
        action_daily(config, args.dry_run, args.limit, args.segment)
